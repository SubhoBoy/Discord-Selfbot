...
def parse_cmd_arguments():  # allows for arguments
    ...
    parser.add_argument(
        "-test",
        "--test-run",  # test run flag for Travis
        action="store_true",
        help="Makes the bot quit before trying to log in",
    )
    parser.add_argument(
        "--force-mac",  # Allows for Testing of mac related code
        action="store_true",
        help="Forces to run the Mac checks",
    )
    parser.add_argument(
        "--reset-config",  # Allows for Testing of mac related code
        action="store_true",
        help="Reruns the setup",
    )
    parser.add_argument(
        "-s",
        "--silent",  # Allows for Testing of mac related code
        action="store_true",
...
if _test_run:
    try:
        ...
        print(
            "Something when wrong. Check for missing sample files"
...
def wizard():
    ...
    print(
        "Then, go into the Applications tab (you may have to click the arrow at the top right to get there), expand the 'Local Storage' dropdown, select discordapp, and then grab the token value at the bottom. Here's how it looks: https://imgur.com/h3g9uf6"
    )
...
if _reset_cfg and not heroku:
    ...
    try:
        ...
        if not heroku:
            ...
            print(
                "Error: Heroku environment detected, but config.json not found!\nThis is usually due to user error during Heroku setup."
            )
            exit(-1)
...
def set_log():
    ...
    errhandler = logging.handlers.RotatingFileHandler(
        filename="settings/logs/bot.log",
        encoding="utf-8",
        mode="a",
        maxBytes=10**7,
        backupCount=5,
...
bot = commands.Bot(
    command_prefix=get_config_value("config", "cmd_prefix"),
    description="""Selfbot by appu1232""",
    self_bot=True,
)
...
async def on_ready():
    ...
    bot.game = (
        bot.game_interval
    ) = bot.avatar = bot.avatar_interval = bot.subpro = bot.keyword_found = None
    ...
    if not os.path.isfile("settings/avatars.json"):
        with open("settings/avatars.json", "w", encoding="utf8") as avis:
            json.dump(
                {"password": "", "interval": "0", "type": "random"}, avis, indent=4
            )
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.errors.CommandNotFound):
        ...
        await ctx.send(
            bot.bot_prefix + "You don't have permissions to use that command."
        )
    elif isinstance(error, commands.errors.MissingRequiredArgument):
        formatter = commands.formatter.HelpFormatter()
        help = await formatter.format_help_for(ctx, ctx.command)
        await ctx.send(
            bot.bot_prefix + "You are missing required arguments.\n" + help[0]
        )
        ...
        if _silent:
            await ctx.send(
                bot.bot_prefix
                + "An error occurred with the `{}` command.".format(ctx.command.name)
            )
        else:
            await ctx.send(
                bot.bot_prefix
                + "An error occurred with the `{}` command. Check the console for details.".format(
                    ctx.command.name
                )
            )
...
async def restart(ctx):
    ...
    def check(msg):
        if msg:
            return (
                msg.content.lower().strip() == "y" or msg.content.lower().strip() == "n"
    ...
    if latest:
        await ctx.send(
            bot.bot_prefix
            + "There is an update available for the bot. Download and apply the update on restart? (y/n)"
        )
        ...
        if not reply or reply.content.lower().strip() == "n":
            ...
            await ctx.send(
                bot.bot_prefix
                + "Downloading update and restarting (check your console to see the progress)..."
            )
            ...
            try:
                ...
                await ctx.send(
                    "{}Failed to reload module `{}` ``` {}: {} ```".format(
                        bot.bot_prefix, i, type(e).__name__, e
                    )
                )
                l -= 1
        await ctx.send(
            bot.bot_prefix + "Reloaded {} of {} modules.".format(l, len(utils))
        )
...
async def on_message(message):
    ...
    if message.author.id == bot.user.id:
        ...
        try:
            if hasattr(bot, "ignored_servers"):
                if any(
                    str(message.guild.id) == str(guild_id)
                    for guild_id in bot.ignored_servers["servers"]
                ):
                    return
        ...
        if hasattr(bot, "self_log"):
            try:
                if str(message.channel.id) not in bot.self_log:
                    bot.self_log[str(message.channel.id)] = collections.deque(
                        maxlen=100
                    )
                ...
                if response:
                    ...
                    if get_config_value("optional_config", "rich_embed") == "on":
                        if response[0] == "embed" and embed_perms(message):
                            try:
                                if get_config_value(
                                    "optional_config", "customcmd_color"
                                ):
                                    color = int(
                                        "0x"
                                        + get_config_value(
                                    ...
                                    await message.channel.send(
                                        content=None,
                                        embed=discord.Embed(colour=color).set_image(
                                            url=response[1]
                                        ),
                                    )
                                else:
    ...
    # Keyword logging.
    if bot.log_conf["keyword_logging"] == "on" and isinstance(
        ...
        try:
            word_found = False
            if (
                bot.log_conf["allservers"] == "True"
                or str(message.guild.id) in bot.log_conf["servers"]
            ) and (
                str(message.guild.id) not in bot.log_conf["blacklisted_servers"]
                and str(message.channel.id) not in bot.log_conf["blacklisted_channels"]
            ):
                if (
                    ...
                    and (
                        ...
                        and not any(
                            x in str(message.author.id)
                            for x in bot.log_conf["blacklisted_users"]
                        )
                    )
                    and message.author not in bot.user.blocked
                ):
                    ...
                    for x in bot.log_conf["blacklisted_words"]:
                        if "[server]" in x:
                            ...
                            if (
                                bword.strip().lower() in message.content.lower()
                                and str(message.guild.id) == id
                            ):
                                word_found = False
                                break
                        elif "[channel]" in x:
            ...
            if bot.log_conf["user_logging"] == "on":
                if (
                    "{} {}".format(str(message.author.id), str(message.guild.id))
                    in bot.log_conf["keyusers"]
                ):
                    ...
                    "{} all".format(str(message.author.id)) in bot.log_conf["keyusers"]
                ):
                    user = "{} all".format(
            ...
            if word_found is True or user_found:
                if (
                    bot.log_conf["user_location"] != bot.log_conf["log_location"]
                    and bot.log_conf["user_location"] != ""
                    and not word_found
                ):
                    ...
                    for i in range(0, total_context):
                        ...
                        if temp.clean_content:
                            msg += (
                                ...
                                % (
                                    temp.author.name,
                                    temp.created_at.replace(tzinfo=timezone.utc)
                                    .astimezone(tz=None)
                                    .__format__("%x @ %X"),
                                )
                                + temp.clean_content.replace("`", "")
                                + "\n\n"
                            )
                    msg += "User: %s | %s\n" % (
                        message.author.name,
                    ...
                    if part == 1:
                        ...
                        while context:
                            ...
                            if temp[0].clean_content:
                                em.add_field(
                                    name="%s" % temp[0].author.name,
                                    value=temp[0].clean_content,
                                    inline=False,
                                )
                        em.add_field(
                            name="%s" % message.author.name,
                        ...
                        for b, i in enumerate(all_words):
                            if b == 0:
                                if bot.notify["type"] == "msg":
                                    await webhook(
                                        bot.bot_prefix
                                        + "%s in server: ``%s`` Context: Channel: <#%s> | %s\n\n```%s```"
                                        % (
                                            logged_msg,
                                            str(message.guild),
                                            str(message.channel.id),
                                    ...
                                    await guild.get_channel(int(location[0])).send(
                                        bot.bot_prefix
                                        + "%s in server: ``%s`` Context: Channel: <#%s>\n\n```%s```"
                                        % (
                                            logged_msg,
                                            str(message.guild),
                                            str(message.channel.id),
                                ...
                                if bot.notify["type"] == "msg":
                                    await webhook(
                                        "```%s```" % i, "message", is_separate
                                    )
                                elif bot.notify["type"] == "ping":
                                    await webhook(
                                        "```%s```" % i, "message ping", is_separate
                                    )
                                else:
                                    await guild.get_channel(int(location[0])).send(
                                        "```%s```" % i
    ...
    if send_type.startswith("embed"):
        if "ping" in send_type:
            await request_webhook(
                ...
                embeds=[keyword_content.to_dict()],
        ...
        if "ping" in send_type:
            ...
            await request_webhook(
                "/{}/{}".format(channel, token), content=keyword_content, embeds=None
            )
                    ...
                    if bot.game_interval:
                            ...
                            if games["type"] == "random":
                                ...
                                if bot.is_stream and "=" in games["games"][next_game]:
                                    ...
                                    await bot.change_presence(
                                        ...
                                        status=set_status(bot),
                                        afk=True,
                                    )
                                else:
                                    await bot.change_presence(
                                        activity=discord.Activity(
                                            name=games["games"][next_game],
                                            type=bot.status_type,
                                        ),
                                        status=set_status(bot),
                                        afk=True,
                                    )
                        ...
                        if game_check:
                            ...
                            if bot.is_stream and "=" in games["games"]:
                                ...
                                await bot.change_presence(
                                    activity=discord.Activity(
                                        name=games["games"], type=bot.status_type
                                    ),
                                    status=set_status(bot),
                                    afk=True,
                                )
            ...
            if hasattr(bot, "refresh_time"):
                ...
                if refresh_time:
                    ...
                    if bot.game and bot.is_stream and "=" in bot.game:
                        ...
                        await bot.change_presence(
                            activity=discord.Activity(
                                name=bot.game, type=bot.status_type
                            ),
                            status=set_status(bot),
                            afk=True,
                        )
...
if __name__ == "__main__":
    ...
    if not os.path.exists("custom_cogs"):
        try:
            ...
            text = (
                "Hello! Seems like you ran into this folder and don't know what this is for. This folder is meant to hold various custom cogs you can download.\n\n"
            ...
            with open("custom_cogs/what_is_this.txt", "w") as fp:
                fp.write(text)
            site = requests.get(
                "https://github.com/LyricLy/ASCII/tree/master/cogs"
            ).text
            ...
            print(
                "Failed to transfer custom cogs to custom_cogs folder. Error: %s"
                % str(e)
            )
        ...
        try:
            ...
            if not heroku:
                if _silent:
                    ...
                    exit(0)
                print(
                    "It seems the token you entered is incorrect or has changed. If you changed your password or enabled/disabled 2fa, your token will change. Grab your new token. Here's how you do it:\n"
                )
                print(
                    "Go into your Discord window and press Ctrl+Shift+I (Ctrl+Opt+I can also work on macOS)"
                )
