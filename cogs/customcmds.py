...
class Customcmds:
    ...
    async def githubUpload(self, username, password, repo_name):
        ...
        repo.update_file(
            "/settings/commands.json", "Updating customcommands", contents, sha
        )
    ...
    async def customcommands(self, ctx):
        ...
        pre = cmd_prefix_len()
        if (
            ctx.message.content[10 + pre :].strip()
            and ctx.message.content[10 + pre :].strip() != "haste"
        ):
            one_cmd = True
        ...
        with open("settings/commands.json") as commands:
            if "haste" in ctx.message.content or "Haste" in ctx.message.content:
                ...
                return await ctx.send(
                    self.bot.bot_prefix + "Full commands.json: %s" % link
                )
            else:
    ...
    async def color(self, ctx, *, msg: str = None):
        ...
        if msg:
            ...
            await ctx.send(
                self.bot.bot_prefix + "Successfully set color for customcmd embeds."
            )
        else:
            msg = ""
            await ctx.send(
                self.bot.bot_prefix + "Removed embed color for customcmd embeds."
            )
    ...
    async def update(self, ctx):
        ...
        with open("settings/github.json", "r+") as fp:
            ...
            if opt["username"] != "":
                try:
                    await self.githubUpload(
                        opt["username"], opt["password"], opt["reponame"]
                    )
                except:
    ...
    async def embed(self, ctx):
        ...
        with open("settings/optional_config.json", "r+") as fp:
            ...
            if opt["rich_embed"] == "on":
                ...
                await ctx.send(
                    self.bot.bot_prefix
                    + "Turned off auto-embeding images/gifs for customcmds."
                )
            else:
                opt["rich_embed"] = "on"
                await ctx.send(
                    self.bot.bot_prefix
                    + "Turned on auto-embeding images/gifs for customcmds."
                )
            fp.seek(0)
            fp.truncate()
            json.dump(opt, fp, indent=4)
    ...
    async def add(self, ctx, *, msg: str = None):
        ...
        if not msg:
                        ...
                        if reply:
                            ...
                            with open("settings/commands.json", "r+") as commands:
                                ...
                                try:
                                    ...
                                    await menu.edit(
                                        content=self.bot.bot_prefix
                                        + "Successfully added ``{}`` to ``{}`` Invoke this response by doing: ``{}``".format(
                                            entry_response,
                                            entry_cmd,
                                    ...
                                    if reply:
                                        ...
                                        with open(
                                            ...
                                            try:
                                                ...
                                                await menu.edit(
                                                    content=self.bot.bot_prefix
                                                    + "Successfully added response with response name ``{}`` to command ``{}`` Invoke this specific response with ``{}`` or get a random response from the list of responses for this command with ``{}``".format(
                                                        entry_response,
                                                        entry_cmd,
                                                        customcmd_prefix
                                                        + entry_cmd
            ...
            try:
                ...
                await ctx.send(
                    self.bot.bot_prefix
                    + "Successfully added ``%s`` to ``%s``" % (entry[1], entry[0])
                )
        ...
        if not msg:
            ...
            if reply:
                ...
                if reply.content == "1":
                    ...
                    if count == 0:
                        return await menu.edit(
                            content=self.bot.bot_prefix + "There are no cmds to remove."
                        )
                    await menu.edit(
                        content=self.bot.bot_prefix
                    ...
                    if reply:
                        ...
                        with open("settings/commands.json", "r+") as commands:
                            ...
                            try:
                                ...
                                await menu.edit(
                                    content=self.bot.bot_prefix
                                    + "Successfully removed command ``{}``".format(
                                        cmd_to_remove
                                    )
                                )
                            except Exception as e:
                ...
                # Remove a specific response from a cmd
                    ...
                    if count == 0:
                        return await menu.edit(
                            content=self.bot.bot_prefix
                            + "There are no cmds with multiple responses. If you are looking to remove a cmd with just one response, select 1 in the main menu for this command."
                        )
                    await menu.edit(
                        content=self.bot.bot_prefix
                    ...
                    if reply:
                        ...
                        if reply:
                            ...
                            with open("settings/commands.json", "r+") as commands:
                                ...
                                commands.truncate()
                                try:
                                    response_to_remove = cmd_responses[
                                        int(reply.content) - 1
                                    ]
                                    ...
                                    await menu.edit(
                                        content=self.bot.bot_prefix
                                        + "Successfully removed response with name ``{}`` from command ``{}``".format(
            ...
            try:
                    ...
                    # Item for key is string
                        ...
                        if entry[0] in cmds:
                            if type(cmds[entry[0]]) is list:
                                await ctx.send(
                                    self.bot.bot_prefix
                                    + "This will delete all responses for this list command. Are you sure you want to do this? (y/n)."
                                )
                ...
                if success is False:
                    await ctx.send(
                        self.bot.bot_prefix + "Could not find specified command."
                    )
