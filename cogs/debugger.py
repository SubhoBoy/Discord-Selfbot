...
class Debugger:
        ...
        try:
            ...
            if result:
                if len(str(result)) > 1950:
                    ...
                    result = (
                        self.bot.bot_prefix
                        + "Large output. Posted to Hastebin: %s" % url
                    )
    ...
    async def debug(self, ctx, *, option: str = None):
        ...
        try:
            if embed_perms(ctx.message):
                em = discord.Embed(
                    color=0xAD2929,
                    title="\ud83e\udd16 Appu's Discord Selfbot Debug Infos",
                )
                system = ""
                if sys.platform == "linux":
                    system = (
                        subprocess.run(["uname", "-a"], stdout=subprocess.PIPE)
                        .stdout.decode("utf-8")
                        .strip()
                    )
                ...
                try:
                    ...
                    _ver = re.search(r"Version: (\d+.\d+.\w+)", str(foo.stdout)).group(
                        1
                    )
                except:
                ...
                em.add_field(
                    name="PIP Version",
                    value="%s" % pkg_resources.get_distribution("pip").version,
                )
                ...
                if option and "cogs" in option.lower():
                    if len(loaded_cogs) > 0:
                        em.add_field(
                            name="Loaded Cogs ({})".format(len(loaded_cogs)),
                            value="\n".join(sorted(loaded_cogs)),
                            inline=True,
                        )
                    if len(unloaded_cogs) > 0:
                        em.add_field(
                            name="Unloaded Cogs ({})".format(len(unloaded_cogs)),
                ...
                if option and "path" in option.lower():
                    ...
                    if len(paths) > 300:
                        ...
                        em.add_field(
                            name="Import Paths",
                            value=paths[:300] + " [(Show more)](%s)" % url,
                        )
                    else:
    ...
    async def save(self, ctx, *, msg):
        ...
        if not os.path.exists("%s/cogs/utils/temp.txt" % os.getcwd()):
            return await ctx.send(
                self.bot.bot_prefix + "Nothing to save. Run a ``>py`` cmd/script first."
            )
        ...
        if os.path.exists("%s/cogs/utils/save/%s.txt" % (os.getcwd(), msg)):
            await ctx.send(
                self.bot.bot_prefix
                + "``%s.txt`` already exists. Overwrite? ``y/n``." % msg
            )
            reply = await self.bot.wait_for(
                "message",
                check=lambda m: m.author == ctx.message.author
                and (m.content.lower() == "y" or m.content.lower() == "n"),
            )
        ...
        try:
            shutil.move(
                "%s/cogs/utils/temp.txt" % os.getcwd(),
                "%s/cogs/utils/save/%s.txt" % (os.getcwd(), msg),
            )
            await ctx.send(
                self.bot.bot_prefix + "Saved last run cmd/script as ``%s.txt``" % msg
            )
        except:
            await ctx.send(
                self.bot.bot_prefix + "Error saving file as ``%s.txt``" % msg
            )
    ...
    async def run(self, ctx, *, msg):
        ...
        if not os.path.exists("%s/cogs/utils/save/%s.txt" % (os.getcwd(), save_file)):
            return await ctx.send(
                self.bot.bot_prefix + "Could not find file ``%s.txt``" % save_file
            )
        ...
        try:
            if txt:
                ...
                if numb.isdigit():
                    ...
                    await ctx.send(
                        self.bot.bot_prefix + "Invalid syntax. Ex: ``>py list 1``"
                    )
            else:
                numb = 1
            ...
            await ctx.send(
                self.bot.bot_prefix
                + "List of saved cmd/scripts. Page ``%s of %s`` ```%s```"
                % (numb, pages, msg)
            )
        except Exception as e:
            await ctx.send(
                self.bot.bot_prefix + "Error, something went wrong: ``%s``" % e
            )
    ...
    async def delete(self, ctx, *, msg: str):
        ...
        try:
            if os.path.exists("cogs/utils/save/%s.txt" % msg):
                ...
                await ctx.send(
                    self.bot.bot_prefix + "Deleted ``%s.txt`` from saves." % msg
                )
            else:
                await ctx.send(self.bot.bot_prefix + "``%s.txt`` does not exist." % msg)
        except Exception as e:
    ...
    async def load(self, ctx, *, msg):
        ...
        try:
            ...
            await ctx.send(
                self.bot.bot_prefix + "Failed to load module: `{}.py`".format(msg)
            )
    ...
    async def unload(self, ctx, *, msg):
        ...
        try:
            ...
            await ctx.send(
                self.bot.bot_prefix + "Failed to unload module: `{}.py`".format(msg)
            )
            await ctx.send(self.bot.bot_prefix + "{}: {}".format(type(e).__name__, e))
        else:
    ...
    async def redirect(self, ctx):
        ...
        await ctx.send(
            self.bot.bot_prefix
            + "Successfully redirected STDOUT and STDERR to the current channel!"
        )
    ...
    async def unredirect(self, ctx):
        ...
        await ctx.send(
            self.bot.bot_prefix
            + "Successfully redirected STDOUT and STDERR back to the console!"
        )
