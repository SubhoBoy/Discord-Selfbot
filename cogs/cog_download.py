...
class CogDownloading:
    ...
    async def github_upload(self, username, password, repo_name, link, file_name):
        ...
        repo.create_file(
            "/custom_cogs/" + file_name,
            "Commiting file: " + file_name + " to GitHub",
            filecontent,
        )
    ...
    async def cog(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            await ctx.send(
                self.bot.bot_prefix
                + "Invalid usage. Valid subcommands: `list`, `install`, `uninstall`, `view`, `update`\nDo `help cog` for more information."
            )
    ...
    async def install(self, ctx, cog):
        ...
        response = requests.get(
            "https://lyricly.github.io/ASCII/cogs/{}.json".format(cog)
        )
        if response.status_code == 404:
            await ctx.send(
                self.bot.bot_prefix
                + "That cog couldn't be found on the network. Check your spelling and try again."
            )
        else:
            cog = response.json()
            ...
            await ctx.send(
                self.bot.bot_prefix
                + "Are you sure you want to download this cog? (y/n)",
                embed=embed,
            )
            ...
            if reply.content.lower() == "y":
                ...
                if "dependencies" in cog:
                    for dep in cog["dependencies"]:
                        try:
                            ...
                            await ctx.send(
                                "{}Warning: dependency {} could not be resolved. Cog may not function as intended".format(
                                    self.bot.bot_prefix, dep
                                )
                            )
                with open("settings/github.json", "r+") as fp:
                    ...
                    if opt["username"] != "":
                        try:
                            await ctx.send(
                                self.bot.bot_prefix
                                + "Uploading to GitHub. Heroku users, wait for the bot to restart."
                            )
                            await self.github_upload(
                                opt["username"],
                                opt["password"],
                                opt["reponame"],
                                coglink,
                ...
                try:
                    self.bot.unload_extension(
                        "custom_cogs." + filename.rsplit(".", 1)[0]
                    )
                    self.bot.load_extension("custom_cogs." + filename.rsplit(".", 1)[0])
                    await ctx.send(
                        self.bot.bot_prefix
                        + "Successfully downloaded the `{}` cog.".format(cog["title"])
                    )
                except Exception as e:
                    os.remove("custom_cogs/" + filename)
                    await ctx.send(
                        self.bot.bot_prefix
                        + "There was an error loading your cog: `{}: {}` You may want to report this error to the author of the cog.".format(
                ...
                await ctx.send(
                    self.bot.bot_prefix
                    + "Didn't download `{}`: user cancelled.".format(cog["title"])
                )
    ...
    async def uninstall(self, ctx, cog):
        ...
        if response.status_code == 404:
            ...
            if os.path.isfile("custom_cogs/" + filename + ".py"):
                embed = discord.Embed(
                    title=found_cog["title"], description=found_cog["description"]
                )
                embed.set_author(name=found_cog["author"])
                await ctx.send(
                    self.bot.bot_prefix
                    + "Are you sure you want to delete this cog? (y/n)",
                    embed=embed,
                )
                reply = await self.bot.wait_for("message", check=check)
                if reply.content.lower() == "y":
                    ...
                    await ctx.send(
                        self.bot.bot_prefix
                        + "Successfully deleted the `{}` cog.".format(
                            found_cog["title"]
                        )
                    )
                else:
                    await ctx.send(
                        self.bot.bot_prefix
                        + "Didn't delete `{}`: user cancelled.".format(
                            found_cog["title"]
                        )
                    )
            else:
                await ctx.send(
                    self.bot.bot_prefix
                    + "You don't have `{}` installed!".format(found_cog["title"])
                )
    ...
    async def list(self, ctx):
        ...
        for entry in list_[3:]:
            response = requests.get(
                "https://lyricly.github.io/ASCII/cogs/{}".format(entry)
            )
            found_cog = response.json()
            filename = found_cog["link"].rsplit("/", 1)[1].rsplit(".", 1)[0]
            if os.path.isfile("custom_cogs/" + filename + ".py"):
    ...
    async def update(self, ctx):
        ...
        for entry in list[2:]:
            ...
            if os.path.isfile("custom_cogs/" + entry + ".py"):
                async with self.bot.session.get(
                    "https://lyricly.github.io/ASCII/cogs/{}.json".format(entry)
                ) as resp:
                    cog = await resp.json()
                    link = cog["link"]
                ...
                try:
                    ...
                    await ctx.send(
                        self.bot.bot_prefix
                        + "There was an error updating the `{}` cog: `{}: {}` You may want to report this error to the author of the cog.".format(
                            cog["title"], type(e).__name__, str(e)
                        )
                    )
        if failures == 0:
            await msg.edit(
                content=self.bot.bot_prefix + "Updated all cogs successfully."
            )
        else:
            await ctx.send(
                self.bot.bot_prefix
                + "Updated {}/{} cogs successfully.".format(
                    successful, successful + failures
                )
            )
