...
class KeywordLogger:
    ...
    async def log_location(self, ctx, msg):
        if msg:
            ...
            if not channel:
                return await ctx.send(
                    self.bot.bot_prefix
                    + "Could not find specified channel. Are you entering in the channel id correctly? Example id: 299293492645986307"
                )
        else:
        ...
        await ctx.send(
            self.bot.bot_prefix
            + "Set log location to channel: ``{}``.".format(channel.name)
        )
        with open("settings/log.json", "r") as log:
    ...
    async def notify_ping(self, ctx):
        ...
        if location == "":
            return await ctx.send(
                self.bot.bot_prefix
                + "Set the channel where you want to keyword log first! See the **Keyword Logger** section in the README for instructions on how to set it up."
            )
        with open("settings/notify.json", "r+") as n:
            notify = json.load(n)
            notify["type"] = "ping"
        ...
        await ctx.send(
            self.bot.bot_prefix
            + "Set notification type to ``ping``. The webhook will ping you."
        )
        if self.bot.subpro:
            self.bot.subpro.kill()
        if os.path.exists("notifier.txt"):
    ...
    async def notify_msg(self, ctx):
        ...
        await ctx.send(
            self.bot.bot_prefix
            + "Set notification type to ``msg``. The webhook will send notifications to your log location channel. Make sure you have notifications enabled for all messages in that channel."
        )
        if self.bot.subpro:
            self.bot.subpro.kill()
        if os.path.exists("notifier.txt"):
    ...
    async def notify_dm(self, ctx):
        ...
        with open("settings/notify.json", "r+") as n:
            ...
            if notify["bot_token"] == "":
                return await ctx.send(
                    self.bot.bot_prefix
                    + "Missing bot token. You must set up a second bot in order to receive notifications (selfbots can't ping themselves!). Read the ``Notifier Setup`` in the Keyword Logger section of the README for step-by-step instructions."
                )
            if notify["type"] == "dm":
                return await ctx.send(
                    self.bot.bot_prefix + "Proxy notifier bot is already on."
                )
            notify["type"] = "dm"
            channel = location
        ...
        await ctx.send(
            self.bot.bot_prefix
            + "Set notification type to ``direct messages``. The proxy bot will direct message you."
        )
        if self.bot.subpro:
            self.bot.subpro.kill()
        if os.path.exists("notifier.txt"):
    ...
    async def webhook_url(self, ctx, msg):
        ...
        await ctx.send(
            self.bot.bot_prefix + "Set up webhook for keyword notifications!"
        )
        with open("settings/log.json", "r") as log:
            self.bot.log_conf = json.load(log)
    ...
    async def log(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            log_loc = (
                ...
                else "Change log location."
            )
            notifier = (
                "Set up notifier to ping you for keywords logged."
                if self.bot.log_conf["webhook_url"] == ""
                and self.bot.log_conf["notifier_bot_token"] == ""
                else "Change settings for keyword notifier."
            )
            ...
            menu = await ctx.send(
                ...
                + "```\n\ud83d\udcdd Keyword Logger Settings. Enter a number:\n\n1. Turn on/off logger (currently {}).\n2. {}\n3. Add/remove a keyword.\n4. Blacklist words/users/server/channels.\n5. {}\n6. User following options.\n7. See all current settings.\n8. Help.```".format(
                    self.bot.log_conf["keyword_logging"], log_loc, notifier
                )
            )
            ...
            if reply and not reply.content.startswith(pre):
                ...
                if int(reply.content) > 2 and self.bot.log_conf["log_location"] == "":
                    return await menu.edit(
                        ...
                        + "Please set a log location (option 2) before setting up the notifier. See the help section (option 8) to for help on understanding the keyword logger better."
                    )
                ...
                if reply.content == "1":
                    ...
                    return (
                        ...
                        if self.bot.log_conf["keyword_logging"] == "on"
                        else await self.start_logger(ctx)
                    )
                    ...
                    if reply and not reply.content.startswith(pre):
                        ...
                        if reply.content == "1":
                            ...
                            if reply and not reply.content.startswith(pre):
                                ...
                                word = (
                                    "[isolated]" + reply.content[1:-1]
                                    if reply.content.startswith('"')
                                    and reply.content.endswith('"')
                                    else reply.content
                                )
                                ...
                                if reply and not reply.content.startswith(pre):
                                    ...
                                    if reply.content == "1":
                                        ...
                                        await menu.edit(
                                            content=self.bot.bot_prefix
                                            + "Keyword ``{}`` successfully set.".format(
                                                o_word
                                            )
                                        )
                                    elif reply.content == "2":
                                        ...
                                        await menu.edit(
                                            content=self.bot.bot_prefix
                                            + "Keyword ``{}`` successfully set for this server.".format(
                                                o_word
                                            )
                                        )
                                    else:
                                        ...
                                        await menu.edit(
                                            content=self.bot.bot_prefix
                                            + "Keyword ``{}`` successfully set for this channel.".format(
                                                o_word
                                            )
                                        )
                        ...
                        # Show list of keywords in order to remove.
                            ...
                            if reply and not reply.content.startswith(pre):
                                ...
                                await menu.edit(
                                    content=self.bot.bot_prefix
                                    + "Successfully removed ``{}`` from your keywords.".format(
                                        word
                                    )
                                )
                        ...
                        # Show list of keywords
                            ...
                            msg = msg.rstrip(", ")
                            await menu.edit(
                                content=self.bot.bot_prefix + "```{}```".format(msg)
                            )
                                    ...
                                    if reply and not reply.content.startswith(pre):
                                        ...
                                        if reply and not reply.content.startswith(pre):
                                            ...
                                            if not guild:
                                                ...
                                                await menu.edit(
                                                    content=self.bot.bot_prefix
                                                    + "Added ``{}`` to the blacklist for server ``{}``.".format(
                                    ...
                                    if reply and not reply.content.startswith(pre):
                                        ...
                                        if reply and not reply.content.startswith(pre):
                                            ...
                                            if not channel:
                                                ...
                                                await menu.edit(
                                                    content=self.bot.bot_prefix
                                                    + "Added ``{}`` to the blacklist for channel ``{}``.".format(
                        ...
                        # Blacklist a user
                            ...
                            if reply and not reply.content.startswith(pre):
                                ...
                                if not user:
                                    await menu.edit(
                                        content=self.bot.bot_prefix
                                        + "Could not find the user. Are they in this server?"
                                    )
                                else:
                                    ...
                                    await menu.edit(
                                        content=self.bot.bot_prefix
                                        + "Blacklisted user ``{}``. This person cannot trigger the keyword logger now.".format(
                                            user.name
                                        )
                                    )
                        ...
                        # Blacklist a server
                            ...
                            if reply and not reply.content.startswith(pre):
                                ...
                                guild = (
                                    ctx.message.guild
                                    if reply.content == "1"
                                    else self.bot.get_guild(int(reply.content))
                                )
                                if not guild:
                                    await menu.edit(
                                        content=self.bot.bot_prefix
                                    ...
                                    await menu.edit(
                                        content=self.bot.bot_prefix
                                        + "Blacklisted server ``{}``. The keyword logger will not trigger for messages from this server.".format(
                                            guild.name
                                        )
                                    )
                        ...
                        # Blacklist a channel
                            ...
                            if reply and not reply.content.startswith(pre):
                                ...
                                channel = (
                                    ctx.message.channel
                                    if reply.content == "1"
                                    else self.bot.get_channel(int(reply.content))
                                )
                                if not channel:
                                    await menu.edit(
                                        content=self.bot.bot_prefix
                                    ...
                                    self.bot.log_conf["blacklisted_channels"][
                                        str(channel.id)
                                    ...
                                    await menu.edit(
                                        content=self.bot.bot_prefix
                                        + "Blacklisted channel ``{}`` in server ``{}``. The keyword logger will not trigger for messages from this channel.".format(
                            ...
                            if reply and not reply.content.startswith(pre):
                                ...
                                if reply.content == "1":
                                    for count, word in enumerate(
                                        self.bot.log_conf["blacklisted_words"]
                                    ):
                                        if " [server]" in word:
                                            key, guild = word.split(" [server]")
                                    ...
                                    if reply and not reply.content.startswith(pre):
                                        ...
                                        index = int(reply.content) - 1
                                        word = self.bot.log_conf["blacklisted_words"][
                                            index
                                        ]
                                        del self.bot.log_conf["blacklisted_words"][
                                            index
                                        ]
                                        ...
                                        await menu.edit(
                                            content=self.bot.bot_prefix
                                            + "Successfully removed ``{}`` from the blacklist.".format(
                                                word
                                            )
                                        )
                                ...
                                # Remove a blacklisted user
                                    ...
                                    for count, word in enumerate(
                                        self.bot.log_conf["blacklisted_users"]
                                    ):
                                        user = None
                                    ...
                                    if reply and not reply.content.startswith(pre):
                                        ...
                                        index = int(reply.content) - 1
                                        word = self.bot.log_conf["blacklisted_users"][
                                            index
                                        ]
                                        del self.bot.log_conf["blacklisted_users"][
                                            index
                                        ]
                                ...
                                # Remove a blacklisted server
                                    ...
                                    for count, word in enumerate(
                                        self.bot.log_conf["blacklisted_servers"]
                                    ):
                                        word = "{}  {}. ".format(
                                    ...
                                    if reply and not reply.content.startswith(pre):
                                        ...
                                        index = int(reply.content) - 1
                                        word = self.bot.log_conf["blacklisted_servers"][
                                            index
                                        ]
                                        del self.bot.log_conf["blacklisted_servers"][
                                            index
                                        ]
                                ...
                                # Remove a blacklisted channel
                                    ...
                                    if reply and not reply.content.startswith(pre):
                                        ...
                                        index = int(reply.content) - 1
                                        word = self.bot.log_conf[
                                            "blacklisted_channels"
                                        ][list_of_channels[index]]
                                        del self.bot.log_conf["blacklisted_channels"][
                                            list_of_channels[index]
                                        ]
                ...
                # Keyword notifier settings
                    ...
                    webhook_is_set = (
                        "Set" if self.bot.log_conf["webhook_url"] == "" else "Change"
                    )
                    ...
                    if reply and not reply.content.startswith(pre):
                        ...
                        if reply.content == "1" or reply.content == "2":
                            ...
                            if self.bot.log_conf["webhook_url"] == "":
                                await menu.edit(
                                    content=self.bot.bot_prefix + webhook_info
                                )
                        ...
                        # Set notifications to dm
                            ...
                            if self.bot.log_conf["notifier_bot_token"] == "":
                                await menu.edit(
                                    content=self.bot.bot_prefix + bot_token_info
                                )
                                reply = await self.block_check(ctx)
                                if reply and not reply.content.startswith(pre):
                                    await reply.delete()
                    ...
                    if reply.content == "1":
                        ...
                        if reply and not reply.content.startswith(pre):
                            ...
                            if reply and not reply.content.startswith(pre):
                                ...
                                try:
                                    float(reply.content)
                                    self.bot.log_conf["keyusers"][
                                        "{} all".format(str(user.id))
                                ...
                                await menu.edit(
                                    content=self.bot.bot_prefix
                                    + "Successfully added user: ``{}``".format(
                                        user.name
                                    )
                                )
                                    ...
                                    for guild in self.bot.guilds:
                                        ...
                                        if member:
                                            msg += "{} CD: {} mins  {}. ".format(
                                                str(member.name),
                                                str(
                                                    self.bot.log_conf["keyusers"][user][
                                                        1
                                                    ]
                                                    / 60.0
                                                ),
                        ...
                        if reply and not reply.content.startswith(pre):
                            ...
                            await menu.edit(
                                content=self.bot.bot_prefix
                                + "Successfully removed the user.".format(word)
                            )
                            ...
                            try:
                                ...
                                if " all" in user:
                                    ...
                                    try:
                                        ...
                                        msg += "{} in server: {} CD: {} mins, ".format(
                                            str(member.name),
                                            str(guild.name),
                                            str(
                                                self.bot.log_conf["keyusers"][user][1]
                                                / 60.0
                                            ),
                                        )
                                    except:
                ...
                # Show all settings
                    ...
                    with open("settings/notify.json") as n:
                        ...
                        if self.bot.log_conf["log_location"] == "":
                            ...
                            msg += "%s in server %s\n\n" % (
                                str(guild.get_channel(int(location[0]))),
                                str(guild),
                            )
                        msg += "Keywords: "
                        for word in self.bot.log_conf["keywords"]:
        ...
        if txt:
            if txt.startswith("save"):
                if txt[5:].strip():
                    ...
                    if size.isdigit():
                        save = True
                        skip = 0
                        fetch = await ctx.send(
                            self.bot.bot_prefix + "Saving messages..."
                        )
                    else:
                ...
                def check(msg):
                    if msg:
                        return (
                            msg.content.lower().strip() == "y"
                            or msg.content.lower().strip() == "n"
                ...
                await ctx.send(
                    self.bot.bot_prefix
                    + "Are you sure you want to output all the messages here? ``(y/n)``."
                )
            ...
            if size.isdigit:
                ...
                if isinstance(ctx.channel, discord.abc.PrivateChannel):
                    ...
                    comments = self.bot.all_log[
                        str(ctx.message.channel.id) + " " + str(ctx.message.guild.id)
                    ]
                if len(comments) - 2 - skip < size:
                    ...
                    if size < 0:
                        size = 0
                for i in range(
                    len(comments) - size - 2 - skip, len(comments) - 2 - skip
                ):
                    attachments = "\r\n"
                    if comments[i][0].clean_content.replace("`", "") == comments[i][
                        1
                    ].replace("`", ""):
                        if (
                            comments[i][0].attachments != []
                            or comments[i][0].embeds != []
                        ):
                        ...
                        msg += (
                            ...
                            + comments[i][0].clean_content.replace("`", "")
                            + attachments
                            + "\r\n"
                        )
                    else:
                        msg += (
                            ...
                            % (
                                ...
                                .astimezone(tz=None)
                                .__format__("%x @ %X"),
                                comments[i][1].replace("`", "") + attachments,
                                comments[i][0].clean_content.replace("`", "")
                                + attachments,
                            )
                        )
                if save is True:
                    save_file = "saved_chat_%s_at_%s.txt" % (
                        ctx.message.created_at.__format__("%x").replace("/", "_"),
                        ctx.message.created_at.__format__("%X").replace(":", "_"),
                    )
                    ...
                    if part == 1:
                        await ctx.send(
                            self.bot.bot_prefix
                            + "Showing last ``%s`` messages: ```%s```" % (txt, msg)
                        )
                        await fetch.delete()
                    else:
                        splitList = [
    ...
    async def location2(self, ctx):
        ...
        if settings["user_location"] == settings["log_location"]:
            await ctx.send(
                self.bot.bot_prefix
                + "User follows and keywords will both log in this channel."
            )
        else:
            if settings["webhook_url2"] == "":
                await ctx.send(
                    self.bot.bot_prefix
                    + "User follows will now log in this channel, however, you will need to set up a separate webhook for this. "
                ...
                await ctx.send(
                    self.bot.bot_prefix
                    + "User follows will now log in this channel instead of the keyword logger channel."
                )
        with open("settings/log.json", "r") as log:
            self.bot.log_conf = json.load(log)
    ...
    async def refresh(self, ctx, *, user: str = None):
        ...
        with open("settings/log.json", "r+") as log:
            ...
            if user:
                ...
                await ctx.send(
                    self.bot.bot_prefix
                    + "Refreshed notification cooldown for this user."
                )
            else:
            ...
            if msg.isdigit():
                if 0 < int(msg) < 21:
                    ...
                    with open("settings/log.json", "r") as log:
                        self.bot.log_conf = json.load(log)
                    await ctx.send(
                        self.bot.bot_prefix + "Set context length to ``%s``." % msg
                    )
                else:
    ...
    async def add(self, ctx):
        ...
        with open("settings/log.json", "r+") as log:
            ...
            if str(ctx.message.guild.id) not in settings["servers"]:
                ...
                await ctx.send(
                    self.bot.bot_prefix + "This server is already in the logger."
                )
        with open("settings/log.json", "r") as log:
            self.bot.log_conf = json.load(log)
    ...
    async def remove(self, ctx):
        ...
        with open("settings/log.json", "r+") as log:
            ...
            if str(ctx.message.guild.id) in settings["servers"]:
                ...
                await ctx.send(
                    self.bot.bot_prefix + "This server is not in the logger."
                )
        with open("settings/log.json", "r") as log:
            self.bot.log_conf = json.load(log)

    @log.command(
        pass_context=True, aliases=["removekey", "addblacklist", "removeblacklist"]
    )
    async def addkey(self, ctx):
        """Add a keyword to the keyword logger. See wiki for more info"""
        await ctx.send(
            self.bot.bot_prefix
            + "This is no longer a command. Use ``>log`` instead to change keyword logger/notifier settings."
        )
        ...
        if " | " in msg.strip():
            ...
            if len(data) == 2:
                ...
                if not stalk_servers:
                    return await ctx.send(
                        self.bot.bot_prefix
                        + "Server not found. Are you using the command right?\nSyntax:``>log adduser <user_id> | <minutes>``\nEx: ``>log adduser {} | 60``".format(
            ...
            try:
                ...
                return await ctx.send(
                    self.bot.bot_prefix
                    + "Invalid interval. Enter a number.\nSyntax:``>log adduser <user_id> | <minutes>``\nEx: ``>log adduser {} | 60``".format(
                        str(self.bot.user.id)
                    )
                )
            user = None
            ...
            if not user:
                return await ctx.send(
                    self.bot.bot_prefix
                    + "Could not find specified user. Are you using the command right?\nSyntax:``>log adduser <user_id> | <minutes>``\nEx: ``>log adduser {} | 60``".format(
                        str(self.bot.user.id)
                    )
                )
            ...
            if stalk_servers is True:
                await ctx.send(
                    self.bot.bot_prefix
                    + "Now ~~stalking~~ following ``{}``.".format(user.name)
                )
            else:
                await ctx.send(
                    self.bot.bot_prefix
                    + "Now ~~stalking~~ following ``{}`` in server ``{}``.".format(
                        user.name, self.bot.get_guild(int(stalk_servers.id)).name
                    )
                )
            with open("settings/log.json", "r") as log:
                print("test")
            ...
            return await ctx.send(
                self.bot.bot_prefix
                + "Invalid syntax. Proper use: ``>log adduser <user_id> | <minutes>``\nEx: ``>log adduser {} | 60``".format(
                    str(self.bot.user.id)
                )
            )
    ...
    async def webhook2(self, ctx, *, msg):
        ...
        with open("settings/log.json", "r+") as l:
            ...
            json.dump(log, l, indent=4)
        await ctx.send(
            self.bot.bot_prefix + "Set up webhook for user follow notifications!"
        )
        with open("settings/log.json", "r") as log:
            self.bot.log_conf = json.load(log)
    ...
    # Set notifications to dm
    @notify.command(
        aliases=["pm", "pms", "direct message", "direct messages", "dms"],
        pass_context=True,
    )
