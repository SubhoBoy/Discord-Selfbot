class Menu:
    ...
    class InputSubmenu:
        """A metaclass of the Menu class for submenu options that take input,
        instead of prompting the user to pick an option."""
    ...
    class ChoiceSubmenu:
        """A metaclass of the Menu class for submenu options
        for choosing an option from a list."""
            ...
            if type(current) == self.Submenu:
                ...
                if type(current.options) == dict:
                    ...
                    for idx, option in enumerate(current.options):
                        result += "[{}] {}: {}\n".format(
                            idx + 1, option, current.options[option]
                        )
                        indexes[idx] = option
                else:
                ...
                if type(current.options) == dict:
                    ...
                    current.input_function(
                        reply, current.options[int(reply.content) - 1]
                    )
