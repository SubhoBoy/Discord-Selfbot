...
class Misc:
    ...
    async def about(self, ctx, txt: str = None):
        ...
        if embed_perms(ctx.message) and txt != "short":
            ...
            em.set_footer(
                text="Made by appu1232#2569", icon_url="https://i.imgur.com/RHagTDg.png"
            )
    ...
    async def stats(self, ctx):
        ...
        if embed_perms(ctx.message):
            ...
            em.add_field(name="\u2328 Most Used Cmd", value=most_used_cmd, inline=False)
            em.add_field(name="\U0001F4E4 Msgs sent", value=str(self.bot.icount))
            em.add_field(
                name="\U0001F4E5 Msgs received", value=str(self.bot.message_count)
            )
            em.add_field(name="\u2757 Mentions", value=str(self.bot.mention_count))
            em.add_field(name="\u2694 Servers", value=str(len(self.bot.guilds)))
            em.add_field(name="\ud83d\udcd1 Channels", value=str(channel_count))
            em.add_field(name="\u270F Keywords logged", value=str(self.bot.keyword_log))
            g = "\U0001F3AE Game"
            if "=" in game:
                g = "\ud83c\udfa5 Stream"
            ...
            em.add_field(name="\U0001F4BE Memory usage:", value=mem_usage)
            try:
                ...
                em.add_field(name="\U0001f4bb Update status:", value=status)
            except:
                pass
            await ctx.send(content=None, embed=em)
        else:
            msg = (
                ...
                % (
                    time,
                    str(self.bot.icount),
                    str(self.bot.message_count),
                    str(self.bot.mention_count),
                    str(len(self.bot.guilds)),
                    str(self.bot.keyword_log),
                    game,
                )
            )
        ...
        **All properties and the syntax:**
        ...
        if msg:
            if embed_perms(ctx.message):
                ptext = (
                    title
                ) = description = image = thumbnail = color = footer = author = None
                ...
                for i in embed_values:
                    ...
                    if i.strip().lower().startswith("ptext="):
                        ...
                        if description is None and not i.strip().lower().startswith(
                            "field="
                        ):
                            description = i.strip()
                ...
                if (
                    ...
                    return await ctx.send(
                        content=None, embed=discord.Embed(description=msg)
                    )
                ...
                if color:
                    ...
                    em = discord.Embed(
                        timestamp=timestamp, title=title, description=description
                    )
                for i in embed_values:
                ...
                if author:
                    if "icon=" in author:
                        ...
                        if "url=" in author:
                            em.set_author(
                                name=author.split("url=")[0].strip()[5:],
                ...
                await ctx.send(
                    self.bot.bot_prefix + "No embed permissions in this channel."
                )
        else:
            msg = (
    ...
    async def editembed(self, ctx, msg_id: int):
        ...
        if not msg:
            ...
            if fields:
                for field in fields:
                    result.append(
                        "field=name={} value={} inline={}".format(
            ...
            info_msg = await ctx.send(
                self.bot.bot_prefix
                + "Embed has been turned back into its command form. Make your changes, then type `done` to finish editing."
            )
    ...
    async def embedcolor(self, ctx, *, color: str = None):
        ...
        with open("settings/optional_config.json", "r+") as fp:
            ...
            if color:
                ...
                await ctx.send(
                    self.bot.bot_prefix + "Successfully set color for embeds."
                )
            else:
                opt["embed_color"] = ""
                await ctx.send(
                    self.bot.bot_prefix
                    + "Set default embed color off for embed command. You will now need to specify the color parameter if you want your embed to be colored when using the embed command."
                )
    ...
    async def game(self, ctx, *, game: str = None):
        ...
        if game:
            ...
            if " | " in game:
                ...
                def check(msg):
                    return (
                        msg.content.isdigit() or msg.content.lower().strip() == "n"
                ...
                if reply.content.lower().strip() == "n":
                    ...
                    if interval >= 10:
                        ...
                        if len(games) != 2:
                            ...
                            if (
                                s.content.strip() == "r"
                                or s.content.strip() == "random"
                            ):
                                await ctx.send(
                                    self.bot.bot_prefix
                                    + "{status} set. {status} will randomly change every ``{time}`` seconds".format(
                        ...
                        if loop_type == "ordered":
                            await ctx.send(
                                self.bot.bot_prefix
                                + "{status} set. {status} will change every ``{time}`` seconds".format(
                                    status=message, time=reply.content.strip()
                                )
                            )
            ...
            # Set game if only one game is given.
                ...
                if is_stream and "=" in game:
                    g, url = game.split("=")
                    await ctx.send(
                        self.bot.bot_prefix + "Stream set as: ``Streaming %s``" % g
                    )
                    ...
                    await self.bot.change_presence(
                        activity=discord.Activity(name=game, type=self.bot.status_type)
                    )
    ...
    async def avatar(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            if avi_config["password"] == "":
                return await ctx.send(
                    self.bot.bot_prefix
                    + "Cycling avatars requires you to input your password. Your password will not be sent anywhere and no one will have access to it. "
            ...
            if avi_config["interval"] != "0":
                ...
                if os.listdir("avatars"):
                    await ctx.send(
                        self.bot.bot_prefix
                        + "Enabled cycling of avatars. Input interval in seconds to wait before changing avatars (``n`` to cancel):"
                    )
                    ...
                    if interval.content.lower().strip() == "n":
                        ...
                        return await ctx.send(
                            self.bot.bot_prefix
                            + "Cancelled. Interval is too short. Must be at least 1800 seconds (30 minutes)."
                        )
                    else:
                    ...
                    if len(os.listdir("avatars")) != 2:
                        await ctx.send(
                            self.bot.bot_prefix
                            + "Change avatars in order or randomly? Input ``o`` for order or ``r`` for random:"
                        )
                        cycle_type = await self.bot.wait_for("message", check=check2)
                        if not cycle_type:
                            return
                        if (
                            cycle_type.content.strip() == "r"
                            or cycle_type.content.strip() == "random"
                        ):
                            await ctx.send(
                                self.bot.bot_prefix
                                + "Avatar cycling enabled. Avatar will randomly change every ``%s`` seconds"
                                % interval.content.strip()
                            )
                            loop_type = "random"
                        else:
                            loop_type = "ordered"
                    ...
                    if loop_type == "ordered":
                        await ctx.send(
                            self.bot.bot_prefix
                            + "Avatar cycling enabled. Avatar will change every ``%s`` seconds"
                            % interval.content.strip()
                        )
                    ...
                    self.bot.avatar = (
                        ...
                        if loop_type == "random"
                        else sorted(os.listdir("avatars"))[0]
                    )
                    with open("avatars/%s" % self.bot.avatar, "rb") as fp:
                        await self.bot.user.edit(
                            password=avi_config["password"], avatar=fp.read()
                        )

                else:
                    await ctx.send(
                        self.bot.bot_prefix
                        + "No images found under ``avatars``. Please add images (.jpg .jpeg and .png types only) to that folder and try again."
                    )
    ...
    async def password(self, ctx, *, msg):
        ...
        return await ctx.send(
            self.bot.bot_prefix
            + "Password set. Do ``>avatar`` to toggle cycling avatars."
        )
                ...
                if opt["password"]:
                    if opt["password"] == "":
                        ...
                        try:
                            ...
                            await ctx.send(
                                self.bot.bot_prefix + "You are being rate limited!"
                            )
                else:
                    await ctx.send(
                        "You have not set your password yet in `settings/avatars.json` Please do so and try again"
    ...
    async def ping(self, ctx):
        ...
        if embed_perms(ctx.message):
            ...
            await ctx.send(
                self.bot.bot_prefix
                + "``Response Time: %s ms``" % str(ping.microseconds / 1000.0)
            )
    ...
    async def quotecolor(self, ctx, *, msg):
        ...
        if msg:
            if msg == "auto":
                await ctx.send(
                    self.bot.bot_prefix + "Successfully set color for quote embeds."
                )
            else:
                try:
                    msg = msg.lstrip("#")
            ...
            await ctx.send(
                self.bot.bot_prefix
                + "Use this command to set color to quote embeds. Usage is `>quotecolor <hex_color_value>`"
            )
            return
        with open("settings/optional_config.json", "r+") as fp:
            opt = json.load(fp)
    ...
    async def quote(self, ctx, *, msg: str = ""):
        ...
        channels = [ctx.channel] + [
            ...
            if x != ctx.channel and type(x) == discord.channel.TextChannel
        ]
            ...
            for chan in ctx.guild.channels:
                ...
                for guild in self.bot.guilds:
                    for chan in guild.channels:
                        if (
                            chan.name == channel
                            or str(chan.id) == channel
                            and type(chan) == discord.channel.TextChannel
                        ):
                            channels.append(chan)
                            break
            if not channels:
                return await ctx.send(
                    self.bot.bot_prefix + "The specified channel could not be found."
                )
        ...
        if result:
            if type(result.author) == discord.User:
                ...
                sender = (
                    result.author.nick if result.author.nick else result.author.name
                )
            if embed_perms(ctx.message) and result.content:
                ...
                await ctx.send(
                    "%s - %s```%s```" % (sender, result.created_at, result.content)
                )
            else:
    ...
    async def afk(self, ctx, txt: str = None):
        ...
        with open("settings/optional_config.json", "r+") as fp:
            ...
            await ctx.send(
                self.bot.bot_prefix
                + "Set default afk status. You will now appear as ``{}`` when not on Discord.".format(
                    opt["default_status"]
                )
            )
