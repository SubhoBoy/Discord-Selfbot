...
class Emoji:
    ...
    async def emoji(self, ctx, *, msg):
        ...
        if len(emojis) > 5:
            return await ctx.send(
                self.bot.bot_prefix + "Maximum of 5 emojis at a time."
            )
        ...
        for emoji in emojis:
            ...
            if url == "":
                await ctx.send(
                    self.bot.bot_prefix + "Could not find {}. Skipping.".format(emoji)
                )
                continue
    ...
    async def add(self, ctx, name, url):
        ...
        try:
            ...
            return await ctx.send(
                self.bot.bot_prefix + "The URL you have provided is invalid."
            )
        if response.status_code == 404:
            return await ctx.send(
                self.bot.bot_prefix + "The URL you have provided leads to a 404."
            )
        try:
            emoji = await ctx.guild.create_custom_emoji(
    ...
    async def remove(self, ctx, name):
        ...
        if emote_length == 1:
            await ctx.send(
                self.bot.bot_prefix + "Successfully removed the {} emoji!".format(name)
            )
        else:
            await ctx.send(
                self.bot.bot_prefix
