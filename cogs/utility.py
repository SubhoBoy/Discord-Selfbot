...
class Utility:
    ...
    async def now(self, ctx):
        ...
        if embed_perms(ctx.message):
            ...
            if thebool:
                em.add_field(
                    name="\u23F0 Time", value="{:%H:%M:%S}".format(dandt), inline=False
                )
            else:
                em.add_field(
                    name="\u23F0 Time",
                    value="{:%I:%M:%S %p}".format(dandt),
                    inline=False,
                )
            em.add_field(
                name="\U0001F4C5 Date", value="{:%d %B %Y}".format(dandt), inline=False
            )
            if tzerror:
                em.add_field(
                    name="\u26A0 Warning",
    ...
    async def selfdestruct(self, ctx, *, amount):
        ...
        if not killmsg:
            return await ctx.send(
                self.bot.bot_prefix + "There is no message to destroy."
            )
    ...
    async def delete(self, ctx, txt=None, channel=None):
        ...
        if txt:  # If number of seconds/messages are specified
            ...
            msg = await ctx.message.channel.history(before=ctx.message).get(
    ...
    async def spoiler(self, ctx, *, msg: str):
        ...
        try:
            ...
            await ctx.message.edit(
                ...
                + "".join(
                    map(
                        lambda c: chr(ord("a") + (((ord(c) - ord("a")) + 13) % 26))
                        if c >= "a" and c <= "z"
                        else c,
                        spoiler,
                    )
    ...
    @commands.group(
        pass_context=True, aliases=["hastebin"], invoke_without_command=True
    )
    ...
    async def sauce(self, ctx, *, txt: str = None):
        ...
        if not txt:
            return await ctx.send(
                self.bot.bot_prefix
                + "Input a link to check the source. Ex: ``>sauce http://i.imgur.com/NIq2U67.png``"
            )
        await ctx.message.delete()
        ...
        try:
            ...
            return await ctx.send(
                self.bot.bot_prefix
                + "Exceeded daily request limit. Try again tomorrow, sorry!"
            )
        soup = BeautifulSoup(webpage, "html.parser")
        pretty_soup = soup.prettify()
        em = discord.Embed(
            color=0xAA550F, description="**Input:**\n{}\n\n**Results:**".format(txt)
        )
        try:
        ...
        if sources != "" and sources:
            em.add_field(
                name="Source sites - percent similarity", value=sources, inline=False
            )
    ...
    async def ud(self, ctx, *, msg):
        ...
        async with self.bot.session.get(
            "http://api.urbandictionary.com/v0/define", params={"term": search}
        ) as resp:
            result = await resp.json()
        if not result["list"]:
            await ctx.send(
                self.bot.bot_prefix
                + "{} couldn't be found on Urban Dictionary.".format(msg)
            )
        else:
            try:
                ...
                await ctx.send(
                    self.bot.bot_prefix
                    + "That result doesn't exist! Try >ud {}.".format(msg)
                )
    ...
    async def youtube(self, ctx, *, msg):
        ...
        async with self.bot.session.get(
            "https://www.youtube.com/results", params={"search_query": search}
        ) as resp:
    ...
    async def xkcd(self, ctx, *, comic=""):
        ...
        if site.status_code == 404:
            ...
            async with self.bot.session.get(
                "https://www.google.co.nz/search?&q={}+site:xkcd.com".format(search)
            ) as resp:
        ...
        if site:
            json = site.json()
            embed = discord.Embed(
                title="xkcd {}: {}".format(json["num"], json["title"]),
                url="https://xkcd.com/{}".format(comic),
            )
    ...
    async def whoisplaying(self, ctx, *, game):
        ...
        if len(msg) > 1500:
            ...
            await ctx.send(
                "{}Large output posted to Hastebin: {}".format(
                    self.bot.bot_prefix, hastebin_output
                )
            )
        elif len(msg) == 0:
            ...
            embed = discord.Embed(
                title="Number of people playing {}".format(game), description=msg
            )
            await ctx.send("", embed=embed)
    ...
    async def animate(self, ctx, animation):
        ...
        try:
            ...
            return await ctx.send(
                self.bot.bot_prefix
                + "You don't have that animation in your `anims` folder!"
            )
        if anim:
            try:
                delay = float(anim[0])
    ...
    async def roles(self, ctx, *, user=None):
        ...
        if not member:
            await ctx.send(
                self.bot.bot_prefix
                + "That user couldn't be found. Please check your spelling and try again."
            )
    ...
    async def messagedump(self, ctx, limit, filename, details="yes", reverse="no"):
        ...
        with open(
            "message_dump/" + filename.rsplit(".", 1)[0] + ".txt",
            "w+",
            encoding="utf-8",
        ) as f:
            if reverse == "yes":
    ...
    async def link(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            await ctx.send(
                self.bot.bot_prefix + "Usage: `link <shorten/lengthen> <url>`"
            )
    ...
    async def shorten(self, ctx, url):
        ...
        if r == 200:
            ...
            if response.status_code == 200:
                await ctx.send(
                    self.bot.bot_prefix + "<{}>".format(response.json()["data"]["url"])
                )
            else:
                await ctx.send(
                    self.bot.bot_prefix + "There was an error shortening your URL."
                )
        else:
    ...
    async def poll(self, ctx, *, msg):
        ...
        if len(options) <= 1:
            return await ctx.send(
                self.bot.bot_prefix + "You must have 2 options or more."
            )
        if len(options) >= 11:
            return await ctx.send(
                self.bot.bot_prefix + "You must have 9 options or less."
            )
        if time:
    ...
    async def loaded(self, ctx):
        ...
        custom_cogs = [
            "custom_cogs." + os.path.splitext(f)[0]
            for f in [os.path.basename(f) for f in glob.glob("custom_cogs/*.py")]
        ]
    ...
    async def read(self, ctx, id: int = None):
        ...
        if id:
            ...
            if guild:
                await guild.ack()
                await ctx.send(
                    self.bot.bot_prefix + "Marked {} as read.".format(guild.name)
                )
            else:
            ...
            await ctx.send(
                self.bot.bot_prefix
                + "Marked {} guilds as read.".format(len(self.bot.guilds))
            )
        ...
        try:
            await ctx.guild.create_role(
                ...
                colour=discord.Colour(
                    eval("0x0{}".format(colour.lstrip("#").lstrip("0x")))
                ),
            )
