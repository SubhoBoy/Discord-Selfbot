    ...
    async def serverinfo(self, ctx, *, msg=""):
        ...
        if ctx.invoked_subcommand is None:
            ...
            if embed_perms(ctx.message):
                ...
                em.set_author(
                    name="Server Info", icon_url="https://i.imgur.com/RHagTDg.png"
                )
                ...
                msg = (
                    ...
                    % (
                        ...
                        server.created_at.__format__("%A, %d. %B %Y @ %H:%M:%S"),
                        server.icon_url,
                    )
                )
        ...
        for role in guild_roles:
            if msg.lower() == role.name.lower() or msg == role.id:
                ...
                if len(role.members) > 10:
                    ...
                    em.add_field(
                        name="All users",
                        value="{} users. [List of users posted to Hastebin.]({})".format(
                            len(role.members), url
                        ),
                        inline=False,
                    )
                ...
                em.add_field(
                    name="Created at", value=role.created_at.__format__("%x at %X")
                )
                em.set_thumbnail(
    ...
    async def inviteinfo(self, ctx, *, invite: str = None):
        ...
        if invite:
            for url in re.findall(r"(https?://\S+)", invite):
                try:
                    invite = await self.bot.get_invite(
                        urlparse(url)
                        .path.replace("/", "")
                        .replace("<", "")
                        .replace(">", "")
                    ...
                    return await ctx.send(
                        self.bot.bot_prefix
                        + "Couldn't find valid invite, please double check the link."
                    )
                break
        else:
        ...
        if not invite:
            return await ctx.send(
                self.bot.bot_prefix
                + "Couldn't find an invite in the last 100 messages. Please specify an invite."
            )
        ...
        if invite.id is not None:
            content = (
                self.bot.bot_prefix + "**Information about Invite:** %s" % invite.id
            )
        if invite.revoked is not None:
            data.colour = (
                discord.Colour.red() if invite.revoked else discord.Colour.green()
            )
        if invite.created_at is not None:
            data.set_footer(
                text="Created on {} ({} days ago)".format(
                    invite.created_at.strftime("%d %b %Y %H:%M"),
                    (invite.created_at - invite.created_at).days,
                )
            )
        ...
        if invite.temporary is not None:
            data.add_field(
                name="Temp membership", value="Yes" if invite.temporary else "No"
            )
        if invite.uses is not None:
            data.add_field(
                name="Uses", value="%s / %s" % (invite.uses, invite.max_uses)
            )
        ...
        if invite.guild.name is not None:
            data.add_field(
                name="Guild",
                value="Name: " + invite.guild.name + "\nID: %s" % invite.guild.id,
                inline=False,
            )
        ...
        if invite.channel.name is not None:
            channel = (
                "%s\n#%s" % (invite.channel.mention, invite.channel.name)
                if isinstance(invite.channel, discord.TextChannel)
                else invite.channel.name
            )
            data.add_field(
                name="Channel",
                value="Name: " + channel + "\nID: %s" % invite.channel.id,
                inline=False,
            )
