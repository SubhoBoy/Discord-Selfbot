...
class Imagedump:
    ...
    def check_images(self, message, images, type_of_items):
        if message.attachments:
            yield from (
                ...
                if item.url.lower().endswith(i.strip())
            )
        ...
        if message.embeds:
            for embed in message.embeds:
                ...
                if (
                    ...
                    or data["type"]
                    in {"jpg", "jpeg", "png", "gif", "gifv", "webm", "image"}
        ...
        if ctx.invoked_subcommand is None:
            ...
            if ctx.message.content[9 + pre :].strip():
                ...
                before_msg = (
                    ...
                    type_of_items_msg
                ) = dimensions_msg = ratio_msg = channel_msg = user_msg = ""
                    ...
                    for i in msg:
                        if i.strip().lower().startswith("items="):
                            ...
                            if limit_images.isdigit():
                                ...
                                return await ctx.send(
                                    self.bot.bot_prefix
                                    + "Invalid Syntax. ``items=`` should be the number of images. Ex: ``>imagedump 500 | items=10``"
                                )
                        if i.strip().lower().startswith("dim="):
                            ...
                            if "x" not in dimensions:
                                return await ctx.send(
                                    self.bot.bot_prefix
                                    + "Invalid Syntax. ``dim=`` should be the dimensions of the image in the form WidthxHeight. Ex: ``>imagedump 500 | dim=1920x1080``"
                                )
                        ...
                        if i.strip().lower().startswith("ratio="):
                            ...
                            if ":" not in ratio:
                                return await ctx.send(
                                    self.bot.bot_prefix
                                    + "Invalid Syntax. ``ratio=`` should be the ratio of the image in the form w:h. Ex: ``>imagedump 500 | ratio=16:9``"
                                )
                        ...
                        if i.strip().lower().startswith("before="):
                            try:
                                ...
                                return await ctx.send(
                                    self.bot.bot_prefix
                                    + "Invalid Syntax. ``before=`` should be a date in the format YYYY-MM-DD. Ex: ``>imagedump 500 | before=2017-02-15``"
                                )
                        ...
                        if i.strip().lower().startswith("after="):
                            try:
                                ...
                                return await ctx.send(
                                    self.bot.bot_prefix
                                    + "Invalid Syntax. ``after=`` should be a date in the format YYYY-MM-DD. Ex: ``>imagedump 500 | after=2017-02-15``"
                                )
                        ...
                        if i.strip().lower().startswith("type="):
                            ...
                            for i in type_of_items:
                                if (
                                    ...
                                    return await ctx.send(
                                        self.bot.bot_prefix
                                        + "Invalid Syntax. ``type=`` should be tye type(s) of items to download. Ex: ``>imagedump 500 | type=png`` or ``>imagedump 500 | type=png, gif``"
                                    )
                        ...
                        if i.strip().lower().startswith("channel="):
                            ...
                            if not channel:
                                return await ctx.send(
                                    self.bot.bot_prefix
                                    + "Channel not found. Are you using the right syntax? ``channel=`` should be the channel id. "
                        ...
                        if i.strip().lower().startswith("user="):
                            ...
                            if not user:
                                return await ctx.send(
                                    self.bot.bot_prefix
                                    + "User not found. Are you using the right syntax? ``user=`` should be the user's id. "
                ...
                if not silent:
                    ...
                    await ctx.send(
                        self.bot.bot_prefix
                        + "Downloading all images/gifs/webms from the last {} messages {}\nSaving to ``image_dump/{}`` Check console for progress.\n{}".format(
                            str(limit - 1), which_channel, new_dump, params
                        )
                    )
                start = time.time()
                images = []
                if limit > 100000:
                    print(
                        "Fetching last %s messages (this may take a few minutes)..."
                        % str(limit - 1)
                    )
                else:
                ...
                try:
                    ...
                    return print(
                        "Something went wrong when saving items and the download was stopped. Error posted above."
                    )
                try:
                ...
                if int(failures) != 0:
                    if not silent:
                        await ctx.send(
                            self.bot.bot_prefix
                            + "Done! ``{}`` items downloaded. ``{}`` However, ``{}`` items failed to download. Check your console for more info on which ones were missed. "
                        ...
                        print(
                            "{} items failed to download. See above for missed links. "
                            "Finished in: {} seconds.".format(
                                str(failures), str(round(stop - start, 2))
                            )
                        )
                else:
                    if not silent:
                        await ctx.send(
                            self.bot.bot_prefix
                            + "Done! ``{}`` items downloaded. ``{}`` Finished in: ``{} seconds.``".format(
                                str(total), size, str(round(stop - start, 2))
                            )
                        )
                    else:
                        print(
                ...
                await ctx.send(
                    self.bot.bot_prefix
                    + "Invalid syntax. ``>imagedump <n>`` where n is the number of messages to search in this channel. "
                    "Ex: ``>imagedump 100``\n``>imagedump dir path/to/directory`` if you want to change where images are saved."
                )
    ...
    async def dir(self, ctx, *, msg: str = None):
        ...
        if msg:
            ...
            if os.path.exists(msg):
                ...
                await ctx.send(
                    self.bot.bot_prefix
                    + "Successfully set path. Images will be saved to: ``{}image_dump/``".format(
                        msg
                    )
                )
            else:
                await ctx.send(
                    self.bot.bot_prefix
                    + "Invalid path. Try again. Example: ``>imagedump dir C:/Users/Bill/Desktop``"
                )
        else:
            with open("settings/optional_config.json", "r") as fp:
                opt = json.load(fp)
            ...
            await ctx.send(
                self.bot.bot_prefix
                + "Current imagedump download location: ``{}``".format(
                    path.replace("\\", "/")
                )
            )
        ...
        if os.path.exists("pause.txt"):
            ...
            for i in paused_dls:
                if i.startswith("paused") or i.startswith("urls"):
                    os.remove("cogs/utils/{}".format(i))
        await ctx.send(
            self.bot.bot_prefix + "Cancelled all imagedump processes currently running."
        )
    ...
    async def pause(self, ctx):
        ...
        for i in self.bot.imagedumps:
            if i.poll() is not None:
                ...
                return await ctx.send(
                    self.bot.bot_prefix
                    + "Paused download. ``>imagedump resume`` to resume. Imagedumps can be resumed even after a restart."
                )
        return await ctx.send(
            self.bot.bot_prefix + "No imagedumps processes are running currently."
        )
    ...
    async def resume(self, ctx):
        ...
        if os.path.exists("pause.txt"):
            ...
            await ctx.send(
                self.bot.bot_prefix + "Resumed imagedump. Check console for progress."
            )
