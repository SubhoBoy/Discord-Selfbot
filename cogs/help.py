...
class Help(formatter.HelpFormatter):
    ...
    async def send(self, dest, content=None, embeds=None):
        if len(embeds) == 1:
            embed = embeds[0]
            embed.set_author(
                name="{0} Help Manual".format(self.bot.user.name), icon_url=self.avatar
            )
        ...
        page_msg = await dest.send(
            self.bot.bot_prefix
            + "There are {} help pages. Send a number to see the corresponding page. Send any other message to exit.".format(
                len(embeds)
            )
        )
    ...
    def destination(self):
        return (
            ...
            if self.bot.pm_help
            else self.context.message.channel
        )
    ...
    async def format(self, ctx, command):
            ...
            if command.help:
                ...
                if len(value) > 1024:
                    ...
                    for new_val in list_values:
                        field = {"name": name, "value": new_val, "inline": False}
        ...
        if self.is_bot():
            ...
            for category, commands in itertools.groupby(data, key=category):
                ...
                if len(commands) > 0:
                    for count, subcommands in enumerate(
                        self._add_subcommands(commands)
                    ):
                        field = {"inline": False}
                        if count > 0:
    ...
    async def format_help_for(self, ctx, command_or_bot, reason: str = None):
        ...
        embed.set_author(
            name="{0} Help Manual Page 1".format(self.bot.user.name),
        ...
        for field in emb["fields"]:
            ...
            if len(txt) > 1000:
                ...
                embed = discord.Embed(color=self.color, **emb["embed"])
                embed.set_author(
                    name="{} Help Manual Page {}".format(
                        self.bot.user.name, len(embeds) + 1
                    ),
                    icon_url=self.avatar,
                )
                embed.set_footer(**emb["footer"])
    ...
    async def help(self, ctx, *cmds: str):
        if not ctx.message.author.permissions_in(ctx.channel).embed_links:
            return await ctx.send(
                ...
                + "You don't have permissions to send embeds here. Find a different server/channel where you can embed links and try the help command there."
            )
        ...
        if len(cmds) == 0:
            ...
            if name in self.bot.cogs:
                ...
                if command is None:
                    await self.send(
                        self.destination, embeds=[self.cmd_not_found(name, self.color)]
                    )
                    return
            ...
            for key in cmds[1:]:
                try:
                    ...
                    if command is None:
                        await self.send(
                            self.destination,
                            embeds=[self.cmd_not_found(key, self.color)],
                        )
                        return
                    ...
                    await self.send(
                        ...
                        embeds=[
                            self.simple_embed(
                                title='Command "{0.name}" has no subcommands.'.format(
                                    command
                                ),
                                color=self.color,
                                author=self.author,
